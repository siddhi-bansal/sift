...
from sift.gemini_client import classify_and_score_item, classify_pain_label
...
def test_classify_and_score_item_json_with_markdown_wrapper():
    ...
    with patch("sift.gemini_client.generate_text", return_value="```json\n" + json.dumps(raw) + "\n```"):
        out = classify_and_score_item("body", "title")
...
def test_classify_and_score_item_parse_fallback():
    ...
    with patch("sift.gemini_client.generate_text", return_value="PAIN\n0.7"):
        with patch("sift.gemini_client.classify_pain_label", return_value=("PAIN", 0.7)):
            out = classify_and_score_item("body", "title")
...
def test_classify_and_score_item_invalid_json_triggers_fallback():
    ...
    with patch("sift.gemini_client.generate_text", return_value="not valid json at all"):
        with patch("sift.gemini_client.classify_pain_label", return_value=("OTHER", 0.0)):
            out = classify_and_score_item("body", "title")
